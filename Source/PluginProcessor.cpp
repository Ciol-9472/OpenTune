...
static bool decodeAudioBufferBase64(const juce::String& b64, int numChannels, int numSamples, juce::AudioBuffer<float>& out) {
...
/** 源文件是否仍与当前 44.1kHz 干声 buffer 一致（未分割等），可仅引用路径 */
                                 ...
                                 const juce::File& projectFileOnDisk)
{
    ...
    for (auto trackState : tracksTree) {
        ...
        lt.colour =
            juce::Colour(static_cast<juce::uint32>(static_cast<int>(trackState.getProperty("colour", 0))));
        ...
        for (const auto& clip : track.clips) {
            ...
            if (canReferenceSourceFile) {
                ...
                const juce::File mediaDir =
                    file.getSiblingFile(file.getFileNameWithoutExtension() + "_media");
...
bool OpenTuneAudioProcessor::loadProjectFromFile(const juce::File& file)
{
...

    {
        ...
        const uint64_t assignedNext =
            std::max(globals.nextClipId, maxClipIdSeen > 0 ? maxClipIdSeen + 1 : 1);
